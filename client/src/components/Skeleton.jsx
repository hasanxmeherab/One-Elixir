...
const Shimmer = ({ className = '' }) => (
  <div className={`bg-gradient-to-r from-[#f0f0f0] via-[#e0e0e0] to-[#f0f0f0] bg-[length:200%_100%] animate-shimmer rounded-sm ${className}`} />
);

// Product card skeleton — used in Shop and Home
  ...
  <div className="flex min-h-screen px-[8%] pt-28 pb-20 gap-20 flex-wrap">
    ...
    <div className="flex-1 min-w-[300px] flex flex-col gap-5 pt-10">
      ...
      <div className="flex gap-3 mt-4">
        {[...Array(3)].map((_, i) => <Shimmer key={i} className="w-16 h-8" />)}
      </div>
      <div className="flex gap-3 mt-2">
        <Shimmer className="flex-1 h-12" />
        <Shimmer className="w-32 h-12" />
      </div>
    </div>
  </div>
);

// Home featured collection skeleton
