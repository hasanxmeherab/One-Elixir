...
import { Package, CheckCircle, Truck, Clock, XCircle, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="px-[5%] md:px-[10%] pt-24 pb-20 min-h-[80vh] max-w-[900px] mx-auto">
      ...
      <div className="text-center mb-14">
        ...
        <div className="w-10 h-0.5 bg-black mx-auto mb-4"></div>
      ...
      {order && !loading && (
        <div>
          ...
          {/* Order Meta */}
          <div className="bg-[#f9f9f9] border border-[#eee] p-6 mb-10 flex flex-wrap justify-between gap-4">
            <div>
          ...
          {isCancelled ? (
            ...
            <div className="mb-12">
              ...
              <div className="hidden sm:flex items-center justify-between mb-8 relative">
                ...
                <div className="absolute top-5 left-0 right-0 h-0.5 bg-[#eee] z-0"></div>
                {/* Progress line fill */}
                <div
                  className="absolute top-5 left-0 h-0.5 bg-black z-0 transition-all duration-700"
                    ...
                    <div key={step.key} className="flex gap-4">
                      <div className="flex flex-col items-center">
                        <div className={`w-9 h-9 rounded-full flex items-center justify-center border-2 shrink-0 ${
                          done || active ? 'bg-black border-black text-white' : 'bg-white border-[#ddd] text-[#ccc]'
                        }`}>
                          <Icon size={14} />
                        </div>
                        {!last && <div className={`w-0.5 flex-1 my-1 ${done ? 'bg-black' : 'bg-[#eee]'}`}></div>}
                      </div>
                      <div className="pb-6">
                        <p className={`text-[10px] font-bold tracking-wider ${active || done ? 'text-black' : 'text-[#bbb]'}`}>
                          {step.label}
                        </p>
                        {active && (
                          <p className="text-xs text-[#666] mt-1 leading-relaxed">{step.desc}</p>
                        )}
                      </div>
                    </div>
                  );
              ...
              {/* Current status description — desktop */}
              {currentStepIndex >= 0 && (
          ...
          <div className="text-center">
            <Link
              to="/account"
              className="inline-flex items-center gap-2 text-xs tracking-[2px] text-[#888] hover:text-black transition-colors no-underline"
            >
              ← BACK TO MY ACCOUNT
            </Link>
          </div>
