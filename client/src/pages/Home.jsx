      ...
      <div style={gridStyle}>
        {perfumes.map((p) => (
          <div key={p._id} style={cardStyle}>
            {/* 1. Badge Logic: Shows 'SOLD OUT' if stock is 0 */}
            {p.stock === 0 && (
              <div style={badgeStyle}>SOLD OUT</div>
            )}
            ...
            <Link to={`/product/${p._id}`}>
              <img src={p.image} alt={p.name} style={{ 
                ...
                height: '350px', 
                objectFit: 'cover',
                opacity: p.stock === 0 ? 0.5 : 1 // Dim image if out of stock
              }} />
            </Link>
            ...
            <div style={{ padding: '15px', textAlign: 'center' }}>
              ...
              {/* 2. Button Logic: Disable button if out of stock */}
              <button 
                onClick={() => addToCart(p)} 
                disabled={p.stock === 0}
                style={{ 
