...
const CostCalculator = () => {
  ...
  const handleSave = async () => {
    ...
    if (ingredients.some(i => !i.name || !i.cost))
      return showToast('Fill in all ingredient names and costs');
  ...
  return (
    <div className="p-6 max-w-[1200px] mx-auto">
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed top-6 right-6 z-50 bg-black text-white px-5 py-3 text-xs tracking-widest">
          {toast}
        </div>
      )}
        ...
        <div className="border border-[#eee] p-6">
          ...
          <div className="mb-6">
            ...
            <input type="number" min="1" value={bottles} onChange={e => setBottles(e.target.value)}
              placeholder="e.g. 10"
              className="w-full border border-[#ddd] px-3 py-2.5 text-sm outline-none" />
          </div>

          {/* Ingredients */}
          <div className="mb-6">
            <div className="flex items-center justify-between mb-3">
              <label className="text-[10px] font-bold tracking-[2px] text-[#888]">INGREDIENTS / ITEMS</label>
              ...
              {ingredients.map((ing, i) => (
                <div key={i} className="grid grid-cols-[2fr_1fr_1fr_1fr_auto] gap-2 items-center">
                  <input value={ing.name} onChange={e => updateIngredient(i, 'name', e.target.value)}
                    placeholder="e.g. Oud Oil"
                    className="border border-[#ddd] px-2 py-2 text-xs outline-none" />
                  <input type="number" value={ing.qty} onChange={e => updateIngredient(i, 'qty', e.target.value)}
                    placeholder="50"
                    className="border border-[#ddd] px-2 py-2 text-xs outline-none" />
                  ...
                  <input type="number" value={ing.cost} onChange={e => updateIngredient(i, 'cost', e.target.value)}
                    placeholder="2000"
                    className="border border-[#ddd] px-2 py-2 text-xs outline-none" />
                  <button onClick={() => removeIngredient(i)} disabled={ingredients.length === 1}
                    className="text-[#ccc] hover:text-red-500 disabled:opacity-20 text-lg leading-none cursor-pointer bg-transparent border-none px-1">
                    ×
                  </button>
                </div>
              ))}
          ...
          <div className="mb-6">
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)}
              placeholder="e.g. Winter batch, bought from Chawkbazar..."
              rows={2}
          ...
          <div className="bg-[#fafafa] border border-[#eee] p-5 mb-6">
            ...
            <div className="grid grid-cols-2 gap-y-3">
              {[
                ...
                ['Cost Per Bottle',   costPerBottle > 0 ? `${costPerBottle.toFixed(0).toLocaleString()} TK` : '—'],
                ['Selling Price',     sellingPrice > 0 ? `${sellingPrice.toLocaleString()} TK` : '—'],
                ['Profit Per Bottle', profit > 0 ? `${profit.toFixed(0).toLocaleString()} TK` : '—'],
              ].map(([label, value]) => (
                    ...
                    {deleteConfirm === r._id ? (
                      <div className="flex gap-2">
                        <button onClick={() => handleDelete(r._id)}
                          className="flex-1 bg-red-600 text-white text-[10px] font-bold tracking-wider py-2 cursor-pointer border-none">
                          CONFIRM DELETE
                        </button>
                        <button onClick={() => setDeleteConfirm(null)}
                          className="flex-1 border border-[#ddd] text-[10px] font-bold tracking-wider py-2 cursor-pointer bg-white">
                          CANCEL
                        </button>
                      </div>
                    ) : (
